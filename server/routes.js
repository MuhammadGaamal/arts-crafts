...
const routes = {
  ...
  getUserOrders: async (req, res) => {
    ...
    const query2 = `
            ...
            `;
    const query = `
            SELECT customers.given_name, customers.fullname, customers.address, customers.city, customers.state, customers.zip, customers.phone, customers.date_added, customers.role FROM customers
    ...
    if (result[0].role === 1) {
      const result1 = (await pool.query(query1)).rows;
      console.log('Inside query1');
      res.status(200).json(result1);
    } else {
      const result2 = (await pool.query(query2, [req.user.id])).rows;
